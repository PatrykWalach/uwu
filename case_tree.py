...
def gen_match(cases1: typing.Sequence[terms.ECase]) -> CaseTree:
    ...
    match cases:
        ...
        case [(patterns,body), *_]:
            ...
            return Node(branch_var, branch_pattern.id, vars, gen_match(yes), gen_match(no))
        case _:
...
def branching_heuristic(
    patterns: dict[str, terms.EMatchVariant], cases: list[tuple[dict[str, terms.EMatchVariant],terms.EDo]]
) -> str:
    return max(
        patterns.keys(),
        key=lambda v: [
            ...
            if #isinstance(case, terms.EMatchVariant) and
             v in patterns
        ],
    )
